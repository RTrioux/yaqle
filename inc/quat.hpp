...
namespace yaql
{
    ...
    inline float re(Quat const &q) { return q.re(); }
    inline float norm(Quat const &q) { return q.norm(); }
    inline float norm2(Quat const &q) { return q.norm2(); }
    ...
    inline Vector3D rotate(Quat const &q, float arr[3]) { return q.rotate(arr); }
