...
namespace yaqle
{
    ...
    inline bool isEqual(DQuat const &dq1, DQuat const &dq2) { return dq1.isEqual(dq2); }
    inline bool isNull(DQuat const &dq) { return dq.isNull(); }
    inline DQuat inverse(DQuat const &dq) { return dq.inverse(); }
    inline DQuat conj(DQuat const &dq) { return dq.conj(); }
    inline DQuat dconj(DQuat const &dq) { return dq.dconj(); }
    inline DQuat mconj(DQuat const &dq) { return dq.mconj(); }
    inline Quat hIm(DQuat const &dq) { return dq.hIm(); }
    inline Quat hRe(DQuat const &dq) { return dq.hRe(); }
    // inline float norm(DQuat const &dq) { return dq.norm(); }
    inline DQuat norm2(DQuat const &dq) { return dq.norm2(); }
    ...
    /** Interpolation **/
    DQuat slerp(const DQuat &dq1, const DQuat &dq2, float t);

    /** Operators **/

    template <typename T>
    DQuat operator/(DQuat const &dq, T const &scalar)
    {
    ...
    template <typename T>
    DQuat operator/(T const &scalar, DQuat const &dq)
    {
